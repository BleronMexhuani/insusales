<x-app-layout>
    ...
    <div class="sm:p-12 p-8 sm:ml-64">
        <div class="space-y-6">
            <x-primary-button class="" onclick="location.href='{{ route('users.create') }}'">Create
                User</x-primary-button>
            <div class="bg-white sm:rounded-lg p-7">
                <div class="overflow-auto">
                    <table class="w-full text-sm text-left mt-6 mb-6 rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @foreach ($users as $item)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <td class="px-6 py-4">
                                        {{ isset($item->getRoleNames()[0]) ? $item->getRoleNames()[0] : 'No role assigned' }}
                                    </td>
                                    <td class="px-6 py-4">
                                        @if($item->is_active == 0)
                                    ...
                                    <td class="py-4 px-3 ">
                                        <a href="{{ route('users.edit', ['user' => $item->id]) }}"><svg
